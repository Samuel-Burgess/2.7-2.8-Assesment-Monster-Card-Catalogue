"""Monster card program for AS 2.7-2.8. indev v014 Added comments for future proofing."""
...
def main_menu():
    ...
    if choice == "Add Cards":
        ...
        card_name = find_cards()  # get the found card in a variable so that I can use it several times in the code
        if card_name is None:  # check for a cancelled search
            ...
            if user_choice == "Edit":
                ...
                del cards[card_name]
                eg.msgbox(f"{card_name} has been deleted", f"{card_name} deleted")
                main_menu()
            else:
        ...
        save_y_n = eg.buttonbox("Save data?", "Save", choices=["Yes", "No"])  # ask user if they want to save data
        if save_y_n == "Yes":
            with open('cards_dictionary.pickle', 'wb') as handle:  # open cards_dictionary.pickle file and dump cards
...
def add_cards():  # add cards function gets the user to create a monster card then returns that card
    global cards
    card_name = eg.enterbox("Please enter the name of the card you are adding:", "Add Card")  # get name
    if card_name is None or card_name.strip() == "":
        eg.msgbox("Cancelled card creation", "Cancelled card creation")  # account for cancellation
    else:
...
def edit_card(card_name):
    ...
    while True:
        ...
        if choice == "Edit stat":
            attribute = eg.choicebox("What stat will you edit?", "Edit Stat",  # stat editing menu
            ...
            if attribute is None:
                ...
                card[attribute] = eg.integerbox(f"What will be {card_name}'s {attribute}?", "Assign Value",
                                                upperbound=25, lowerbound=1)  # get new stat value
            ...
            while new_card_name is None:  # loop until a new name is given
                ...
                if new_card_name == original_card_name:  # check if they just input the same name
                    ...
                    eg.msgbox("Please enter a valid name", "Enter a valid name")  # handles all other invalid names
        else:
            break
...
def delete_card():
    if len(cards) > 0:  # check if there are cards to delete, make a choicebox of all the cards
        ...
        if card_name is not None:
            ...
            eg.msgbox("Cancelled delete", "Cancelled delete")  # handle cancellations
        main_menu()
    else:
...
def print_cards():
    ...
    for card_name, card_stats in cards.items():  # go through each card, printing each stat and stat value
